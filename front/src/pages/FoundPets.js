  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // İlanları getir
        const petsResponse = await fetch('http://localhost:5000/api/pets/found');
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Typography variant="subtitle1" color="text.secondary" gutterBottom>
            Sahiplerini arayan dostlarımız
          </Typography>
        </Box>
        <Box>
      ...
      {filteredPets.length === 0 ? (
        <Typography variant="body1" color="text.secondary" sx={{ textAlign: 'center', mt: 4 }}>
          {pets.length === 0 ? 'Henüz bulunan hayvan ilanı bulunmuyor.' : 'Filtrelere uygun ilan bulunamadı.'}
        </Typography>
      ) : (
            ...
            <Grid item xs={12} sm={6} md={4} key={pet.id}>
              ...
              >
                <CardMedia
                  component="img"
                  height="200"
                  image={pet.found_image || '/default-pet.jpg'}
                  alt={`${pet.tur} ${pet.cins}`}
                  sx={{ objectFit: 'cover' }}
                />
                <CardContent>
                  <Box sx={{ mb: 2 }}>
