...
const ReportFound = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Form validasyonu
    if (!formData.tur || !formData.cins || !formData.cinsiyet || !formData.sehir || !formData.ilce) {
    ...
    try {
      ...
      const postData = {
        ...
        email: user.user.email,
        found_image: formData.found_image,
        description: formData.description
      };

      console.log('Gönderilecek veriler:', postData);

      const response = await fetch('http://localhost:5000/api/pets/found', {
        method: 'POST',
        headers: {
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                placeholder="Bulduğunuz hayvanın özellikleri, bulunduğu yer ve zaman hakkında detaylı bilgi verin..."
              />
            </Grid>
